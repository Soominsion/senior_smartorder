...
os.environ['GOOGLE_APPLICATION_CREDENTIALS'] = r"C:\Users\정혜교\Desktop\VenvWorkspace\still-primer-406515-9e17e40c8c06.json"
...
def process_text():
    if request.method == 'POST':
        ...
        if similar_word and similar_word[0] == '커피':
            # '커피'로 판단되면 처리할 작업 수행, 여기에서 리디렉션 또는 원하는 작업을 수행하세요.
            return redirect('/third_coffee')


    # 여기서 처리하지 않은 다른 경우에 대한 기본 응답
    return redirect('/second')
    #return render_template('second.html')


#@app.route('/function_select', methods=['GET','POST'])
#def function_select():
    #if request.method == 'POST':
        #data = {} #데이터 받을 곳
        #data[]
